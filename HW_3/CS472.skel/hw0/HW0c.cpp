...
HW0c::initializeGL()
{
	...
	// init vertex and color buffer ids (only once)
...
HW0c::paintGL()
{
	...
	// bind vertex buffer and enable the assignment of the attribute vertex variable
	glBindBuffer(GL_ARRAY_BUFFER, m_vertexBuffer);
	glEnableVertexAttribArray(ATTRIB_VERTEX);

	// assign buffer object to attribute vertex variable
	glVertexAttribPointer(ATTRIB_VERTEX, 2, GL_FLOAT, false, 0, NULL);

	// bind color buffer and enable the assignment of the attribute color variable
	glBindBuffer(GL_ARRAY_BUFFER, m_colorBuffer);
	glEnableVertexAttribArray(ATTRIB_COLOR);

	// assign buffer object to attribute color variable
